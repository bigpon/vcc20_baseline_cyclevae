...
from parallel_wavegan.datasets import AudioWorldDataset
...
class Trainer(object):
    ...
    def load_pretrainpoint(self, pretrainpoint_path):
        """Load pretrained checkpoint.

        Args:
            pretrainpoint_path (str): Pretrained checkpoint path to be loaded.

        """
        state_dict = torch.load(pretrainpoint_path, map_location="cpu")
        if self.config["distributed"]:
            self.model["generator"].module.load_state_dict(state_dict["model"]["generator"])
            self.model["discriminator"].module.load_state_dict(state_dict["model"]["discriminator"])
        else:
...
def main():
    ...
    parser.add_argument("--audio_train", required=True,
                        type=str, help="list of training wav files")
    parser.add_argument("--world_train", required=True,
                        type=str, help="list of training world feat files")
    parser.add_argument("--audio_dev", required=True,
                        type=str, help="list of development wav files")
    parser.add_argument("--world_dev", required=True,
                        type=str, help="list of development world feat files")
    ...
    if config["remove_short_samples"]:
        world_length_threshold = config["batch_max_steps"] // config["hop_size"] + \
    ...
    dataset = {
        "train": AudioWorldDataset(
            stats=args.stats,
            audio_list=args.audio_train,
            world_list=args.world_train,
            audio_load_fn=audio_load_fn,
            world_load_fn=world_load_fn,
            hop_size=config["hop_size"],
            world_length_threshold=world_length_threshold,
            allow_cache=config.get("allow_cache", False),
            mean_path=config.get("mean_path", "/world/mean"),
            scale_path=config.get("scale_path", "/world/scale"), # keep compatibility
        ),  
        "dev": AudioWorldDataset(
            stats=args.stats,
            audio_list=args.audio_dev,
            world_list=args.world_dev,
            audio_load_fn=audio_load_fn,
            world_load_fn=world_load_fn,
            hop_size=config["hop_size"],
            world_length_threshold=world_length_threshold,
    ...
    if os.path.exists(args.resume): # resume from checkpoint
        trainer.load_checkpoint(args.resume)
        logging.info(f"Successfully resumed from {args.resume}.")
    else:
        logging.info(f"Start a new training process.")
